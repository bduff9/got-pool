...
export default class AdminUsers extends TrackerReact(Component) {
	...
	_deleteUser (user_id, ev) {
		deleteUser.call({ user_id }, displayError);
	}
						...
						{this.users().map(user => (
							<tr key={`log${user._id}`}>
								<td>
									...
									{user.is_admin ? null : <Icon icon="times" hasTextColor="danger" className="clickable" onClick={this._deleteUser.bind(null, user._id)} />}
								</td>
								<td>
